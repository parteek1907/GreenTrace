...
import { mockMonthlyData, mockCarbonScore, mockActivityFeed } from "@/lib/mock-data";
import { formatCO2, categoryColor } from "@/lib/utils/formatters";
import AnimatedCounter from "@/components/charts/AnimatedCounter";
import { staggerContainer, fadeInUp } from "@/lib/utils/animations";

const categoryData = [
  { name: "Transport", current: 175, previous: 230, color: categoryColor("transport") },
  { name: "Food", current: 140, previous: 155, color: categoryColor("food") },
  { name: "Energy", current: 100, previous: 120, color: categoryColor("energy") },
  { name: "Shopping", current: 68, previous: 75, color: categoryColor("shopping") },
  { name: "Waste", current: 37, previous: 40, color: categoryColor("waste") },
];

export default function ProgressPage() {
  const totalCurrent = mockMonthlyData[mockMonthlyData.length - 1].totalKg;
  const totalPrevious = mockMonthlyData[0].totalKg;
    ...
    <motion.div variants={staggerContainer} initial="hidden" animate="visible">
      ...
      <motion.div variants={fadeInUp} className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
        ...
        <div className="glass rounded-xl p-5 border border-white/5 text-center">
          ...
          <div className="text-2xl font-bold text-gt-text">
            <AnimatedCounter value={Math.round(mockMonthlyData.reduce((s, m) => s + m.totalKg, 0) / 12)} suffix=" kg" />
          </div>
        </div>
              ...
              <div key={cat.name}>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="text-gt-success text-xs">
                    ↓ {Math.round(((cat.previous - cat.current) / cat.previous) * 100)}%
                  </span>
                </div>
                <div className="flex gap-1 items-center">
                  <div className="flex-1 h-2 bg-white/5 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      initial={{ width: 0 }}
                      animate={{ width: `${(cat.previous / 250) * 100}%` }}
                ...
                <div className="flex gap-1 items-center mt-1">
                  <div className="flex-1 h-2 bg-white/5 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      style={{ backgroundColor: cat.color }}
                      initial={{ width: 0 }}
                      animate={{ width: `${(cat.current / 250) * 100}%` }}
