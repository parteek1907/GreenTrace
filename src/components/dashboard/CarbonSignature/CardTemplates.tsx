  ...
  <div className={`relative w-[1080px] h-[1920px] flex flex-col justify-between p-16 overflow-hidden`}>
    ...
    <div className="relative z-10 flex justify-between items-start">
      <div className="space-y-3">
        ...
        <p className={`text-[18px] font-black tracking-[0.5em] uppercase ${theme.textPrimary}`}>Signature: GT-82-A</p>
      </div>
    ...
    <div className="relative z-10 flex flex-col justify-center flex-1 py-16">
      ...
      <div className="flex flex-wrap gap-5 mt-4">
        <PremiumBadge label="Top 12% Globally" theme={theme} icon={Globe2} />
        <PremiumBadge label="Earth Steward" theme={theme} icon={Award} />
      </div>
    </div>
    ...
    <div className="relative z-10 flex flex-col justify-center flex-1 py-16 space-y-20">
      ...
      <div className={`p-16 rounded-[60px] ${theme.glass} border-l-[12px] border-l-current backdrop-blur-3xl shadow-2xl relative overflow-hidden`} style={{ borderLeftColor: 'currentColor', color: theme.accent.replace('text-', '') }}>
        ...
        <div className="flex items-baseline gap-4 relative z-10">
          <p className={`text-[200px] font-black tracking-tighter leading-none text-white drop-shadow-2xl`}>25</p>
      ...
      <div className="space-y-6">
        ...
        <p className={`text-[56px] font-bold leading-tight ${theme.textPrimary}`}>
          You saved <span className={`font-black ${theme.accent}`}>684kg CO₂</span>, equal to planting <span className={`font-black ${theme.accent}`}>34 fully-grown trees</span> this year.
        </p>
      </div>

      <div className="flex gap-5">
        <PremiumBadge label="22 Challenges" theme={theme} icon={Flag} />
        <PremiumBadge label="84 Day Streak" theme={theme} icon={Zap} />
      </div>
    </div>
...
export const TwinAchievementCard = ({ theme, profile, quote }: CardProps) => (
  <div className={`relative w-[1080px] h-[1920px] flex flex-col justify-between p-16 overflow-hidden`}>
    <AestheticBackground theme={theme} />
    ...
    <div className="relative z-10 flex flex-col items-center text-center justify-center flex-1 py-16">
        ...
        <div className="relative z-10 space-y-12">
          <div className="flex justify-between items-end">
            <div>
              ...
              <p className={`text-6xl font-bold ${theme.textPrimary}`}>6,840 kg</p>
            </div>
            <TrendingDown className={`w-16 h-16 text-white/20`} />
          </div>
          
          <div className="w-full h-[2px] bg-white/10 relative">
            <div className={`absolute top-0 left-0 h-full w-[65%] bg-current`} style={{ color: theme.accent.replace('text-', '') }}></div>
          </div>

          <div className="flex justify-between items-end">
            <div>
              <p className={`text-2xl font-bold tracking-[0.3em] uppercase ${theme.accent} mb-3`}>Projected Future You</p>
              <p className={`text-[96px] font-black leading-none text-white drop-shadow-lg`}>5,120 kg</p>
            </div>
      ...
      <div className="mt-16">
        <PremiumBadge label="Eco Strategist" theme={theme} icon={Globe2} />
      </div>
    </div>
...
export const CardRenderer = ({ templateId, themeId, profile, quote }: { templateId: string, themeId: string, profile: ProfileData, quote: string }) => {
  const theme = THEMES.find(t => t.id === themeId) || THEMES[0];
  
  switch(templateId) {
    case 'signature':
      return <CarbonSignatureCard theme={theme} profile={profile} quote={quote} />;
    case 'snapshot':
      return <ImpactSnapshotCard theme={theme} profile={profile} quote={quote} />;
    case 'twin':
      return <TwinAchievementCard theme={theme} profile={profile} quote={quote} />;
    default:
      return <CarbonSignatureCard theme={theme} profile={profile} quote={quote} />;
  }
};
